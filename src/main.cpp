...
#if defined(NDEBUG)
# error "Achieverpoint cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa7ff6ddda08bfbd6ebe8f3ee1847a1e735cba839fdd6c01f167425c6ed811c9d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Achieverpoint: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Achieverpoint Signed Message:\n";
...
{
    // Achieverpoint: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 265000); // Achieverpoint: 840k blocks in ~4 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Achieverpoint: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Achieverpoint: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcc317f7acce169a133946c72c4bc3e4f3d1e8f1e44c09acc8c1279fc162841b3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times /2012/11/11/ How a Beach Community Became a Deathtrap";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0443e6d80b340eba52488a0dada52782d9ed97b1110d8a663a09f14dce8f66b2abc788587228730b78d2a41eced1461f8e1d29ef9bf6b6546c3737eee6da468a7f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518031770;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085725345;

        if (fTestNet)
        {
            block.nTime    = 1518031764;
            block.nNonce   = 385442160;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x5bafcd3334fa3ac4301ad3aa4ac99b2c700d28bc929f8a1869f61ab868c80d12"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xe2, 0xd3, 0xc4 }; // Achieverpoint: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AchieverpointMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AchieverpointMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AchieverpointMiner(CWallet *pwallet)
{
    printf("AchieverpointMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("achieverpoint-miner");
    ...
    try { loop {
        ...
        printf("Running AchieverpointMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AchieverpointMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AchieverpointMiner, pwallet));
}
